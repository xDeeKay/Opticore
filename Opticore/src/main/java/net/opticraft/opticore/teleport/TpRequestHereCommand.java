		...
		if (cmd.getName().equalsIgnoreCase("tprequesthere") || cmd.getName().equalsIgnoreCase("tprhere")) {
			if (sender instanceof Player) {
				...
				if (args.length == 1) {
					...
					if (plugin.players.get(senderName).getTprOutgoing() == null && plugin.players.get(senderName).getTprhereOutgoing() == null) {
						...
						if (plugin.getServer().getPlayer(target) != null) {
							...
							if (plugin.players.get(targetName).getSettings().get("teleport_request").getValue() == 1) {
								teleportUtil.teleportRequestHere(senderName, targetName);
							}

							util.sendStyledMessage(senderPlayer, null, "GREEN", "/", "GOLD", "Teleport request here sent to '" + targetName + "'.");
							...
							if (server != null) {
								...
								plugin.players.get(senderName).setTprhereOutgoing(target);

								util.sendStyledMessage(senderPlayer, null, "GREEN", "/", "GOLD", "Teleport request here sent to '" + target + "'.");
					...
					util.sendStyledMessage(senderPlayer, null, "RED", "/", "GOLD", "Incorrect syntax. Usage: /tprhere <player>");
				}
			} else {
